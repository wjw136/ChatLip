    ...
    def _load(
        ...
        if gpt_config_path:
            ...
            if compile and "cuda" in str(device):
                try:
                    compile_options = {
                        "extra_compile_args": ["-std=c99"], # gcc 4.8.5 默认是c89和c++98(c98 c++11)
                    }
                    gpt.forward = torch.compile(
                        gpt.forward, backend="inductor", dynamic=True, options=compile_options
                    )
                    gpt.gpt.forward = torch.compile(
                        gpt.gpt.forward, backend="inductor", dynamic=True, options=compile_options
                    )
